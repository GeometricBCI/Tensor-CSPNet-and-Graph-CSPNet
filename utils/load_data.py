...
class load_BCIC:
    def __init__(self, sub, TorE = True, alg_name ='Tensor_CSPNet', session_no = 1, scenario = 'CV'):
        ...
        if scenario == 'CV':
            ...
            self.train_indices = np.load('index/BCIC_sess'+str(session_no)+'_sub'+str(sub)+'_train_index.npy', allow_pickle=True)
            self.test_indices  = np.load('index/BCIC_sess'+str(session_no)+'_sub'+str(sub)+'_test_index.npy', allow_pickle=True)
